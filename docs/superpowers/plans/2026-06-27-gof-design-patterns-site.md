...
// Wrap image tags with skeleton/fallback behavior at runtime.
const wrappedHtml = computed(() => {
  return html.value.replace(
    /<img\s+([^>]*?)src="([^"]+)"([^>]*?)>/g,
    (_, before, src, after) => {
      const skeleton = `<span class="image-skeleton" aria-hidden="true"></span>`
      const fallback = `<span class="image-fallback" role="img" aria-label="示意图不可用">示意图</span>`
      // Use onerror to swap to fallback; the skeleton is shown only while loading.
      return `<img ${before} src="${src}" ${after} loading="lazy" decoding="async" style="display:none" onload="this.style.display='block';this.previousElementSibling?.remove()" onerror="this.remove();this.insertAdjacentHTML('afterend','${fallback.replace(/'/g, "\\'")}')">${skeleton}`
    },
  )
})
...
<template>
  <article class="prose" @click="onClick" v-html="wrappedHtml" />
</template>
```
